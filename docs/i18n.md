...
|--------|------|---------|-------------|
| `--retries` | `int` | `3` | Number of retries per translation file on failure (minimum 1, maximum 5). Use this to control how aggressive retries should be. |
...
- With `--delay=2 --retries=5`: Up to 5 attempts; if the first 4 are 429s: 32s + 62s + 182s + 302s = ~9.6 minutes, then 1 more retry with the base 2s delay
